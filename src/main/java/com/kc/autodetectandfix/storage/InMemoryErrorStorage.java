package com.kc.autodetectandfix.storage;
...
public class InMemoryErrorStorage implements ErrorStorage {
    ...
    @Override
    public boolean storeError(DetectedError error) {
        ...
        if (existing.isPresent()) {
            ...
            logger.info("Duplicate error detected: {} (occurrence count: {}). NOT sending email.",
                existingError.getId(), existingError.getOccurrenceCount());
            return false;  // Duplicate - do not send notification
        } else {
            // Store new error
            ...
            return true;  // New error - should send notification
        }
    }
